...
{
	matrixWrite(matrixRegDecodeMode, 0x00);		// no decoding
	matrixWrite(matrixRegIntensity, matrixIntensity);	// brightness intensity
	matrixWrite(matrixRegScanLimit, 0x07);		// scan limit = 8 LEDs
	matrixWrite(matrixRegShutdown, 0x01);			// power down=0, normal=1
	matrixWrite(matrixRegDisplayTest, 0x00);		// no test display
}
...
void flash(void)
{
	...
	{
		matrixWrite(matrixRegIntensity,intensity);
		delay(SECOND/20);
	}
	...
	matrixWrite(matrixRegIntensity,matrixIntensity);
}
